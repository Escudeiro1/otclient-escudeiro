-- Line-oriented OTML parser/writer.  (vendored copy of modules/dev_otui/otml.lua;
-- the only change is the global name Otml -> UiInspectorOtml so the two dev tools
-- never share state.)
--
...
function UiInspectorOtml.parse(text)
...
function UiInspectorOtml.widgetChildren(node)
  local out = {}
...
function UiInspectorOtml.properties(node)
  local out = {}
  for _, child in ipairs(node.children) do
...
function UiInspectorOtml.findProperty(node, key)
  for _, child in ipairs(node.children) do
...
function UiInspectorOtml.lastLineOf(node)
  local last = node.line
  for _, child in ipairs(node.children) do
    local childLast = UiInspectorOtml.lastLineOf(child)
...
-- in the tree. Callers must reparse before the next edit (UiInspectorOtml.reparse).

function UiInspectorOtml.reparse(doc)
  local text = UiInspectorOtml.serialize(doc)
  return UiInspectorOtml.parse(text)
end
...
local function insertionLine(node)
  ...
  for _, child in ipairs(node.children) do
    if child.isProp then
      local last = UiInspectorOtml.lastLineOf(child)
...
function UiInspectorOtml.setProperty(doc, node, key, value)
  local prop = UiInspectorOtml.findProperty(node, key)
...
function UiInspectorOtml.removeProperty(doc, node, key)
  local prop = UiInspectorOtml.findProperty(node, key)
  if not prop then
    return false, 'property not present in the file'
  end
  local last = UiInspectorOtml.lastLineOf(prop)
...
function UiInspectorOtml.addChildWidget(doc, parentNode, styleName, id, props)
  if not parentNode then
    return false, 'no parent widget'
  end

  local indent = string.rep(' ', parentNode.indent + 2)
  local at = UiInspectorOtml.lastLineOf(parentNode) + 1
...
function UiInspectorOtml.removeNode(doc, node)
  if not node then
    return false, 'no node given'
  end

  local first = node.line
  local last = UiInspectorOtml.lastLineOf(node)
...
function UiInspectorOtml.skeleton(styleName, id, title)
  return table.concat({
    styleName,
    '  id: ' .. id,
    "  !text: '" .. title .. "'",
...
function UiInspectorOtml.serialize(doc)
