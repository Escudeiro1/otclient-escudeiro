// EXPERIMENTAL / not a "better" xBR - see the requesting conversation.
// This is xbr.frag with the exact-equality bug from the base check
// (irlv0/irlv2l/irlv2u) deliberately restored instead of fixed: under this
// client's bilinear "Smooth Retro" filtering, that makes those checks fire
// on essentially every fragment (pure interpolation jitter, not real edges),
// so almost the whole screen gets *some* blend amount driven by which of two
// weighted-distance sums happens to be smaller at that pixel - closer to a
// fine shimmer/dither texture than genuine edge reconstruction. Kept as its
// own variant specifically to compare against the correct xbr.frag rather
// than replacing it - operator asked to see this look, explicitly informed
// beforehand that it isn't "more real edges detected."
//
