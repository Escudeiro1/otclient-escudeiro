// Kuwahara filter, radius 2 (5x5 neighbourhood, 3x3 quadrants). This is the
// original "Map - HD" shader from before this session's xBR work - restored
// under a clearer name, since it turns out this smoothing/rounding effect
// (not sharper edges) was the actual "HD" look wanted. Produces a strong
// smooth "plastic/rubber" look: flat colour areas become perfectly clean,
// while the boundary between two differently-colored regions stays as a
// clean, moved edge rather than a blur - the algorithm picks whichever of
// four overlapping quadrants around this pixel is most uniform and outputs
// its average, so it never blends straight across a real silhouette edge.
//
// Fix vs. the original: neighbour offsets now scale with u_MapZoom (screen
// pixels per source texel, same fix applied to every other shader added
// this session), so the 5x5 sampling radius correctly tracks real source
// texels regardless of render/zoom scale, instead of shrinking at higher
// internal render scales the way the original's fixed 1/u_Resolution did.
...
void main() {
    vec2 p = vec2(u_MapZoom / u_Resolution.x, u_MapZoom / u_Resolution.y);
